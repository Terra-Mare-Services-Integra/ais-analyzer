...
function MapFit({ points }) {
  ...
  useEffect(() => {
    ...
    map.fitBounds([[Math.min(...lats)-.05,Math.min(...lons)-.05],[Math.max(...lats)+.05,Math.max(...lons)+.05]],{padding:[20,20]});
  }, [points, map]);
  return null;
}
...
export default function TripViewer({ trips, setTrips, initialIdx = 0, onBack }) {
  ...
  const visible = filter === "ALL" ? points : filter === "SVC" ? points.filter(p=>p.state==="WORKING_STOP") : points.filter(p=>p.state!=="IN_PORT");
  ...
  const handleSave = useCallback(async (updated) => {
    ...
    // Persist to Supabase — find the point by datetime + trip
    try {
      const currentTrip = newTrips[tripIdx];
      if (currentTrip.supabaseId) {
        await supabase.from("ais_points")
          .update({ tipo_servicio: updated.tipo_servicio, zona_servicio: updated.zona_servicio })
          .eq("trip_id", currentTrip.supabaseId)
          .eq("datetime", updated.datetime.toISOString ? updated.datetime.toISOString() : new Date(updated.datetime).toISOString());
      }
    } catch (e) {
      console.warn("Error guardando clasificación:", e);
    } finally {
      setSaving(false);
    }
  ...
  const markValidated = async () => {
    ...
    // Persist validated flag
    const currentTrip = trips[tripIdx];
    if (currentTrip.supabaseId) {
      await supabase.from("ais_trips").update({ validated: true }).eq("id", currentTrip.supabaseId);
    }
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 300px",flex:1,minHeight:0 }}>
        ...
        <div style={{ position:"relative",height:"100%" }}>
          <MapContainer center={[-34.7,-58.0]} zoom={9} style={{ height:"100%",width:"100%" }}>
            ...
            {Object.entries(ZONES).map(([key,z]) => (
              <Polyline key={key} positions={[...z.polygon.map(([a,b])=>[a,b]),[z.polygon[0][0],z.polygon[0][1]]]}
        ...
        <div style={{ display:"flex",flexDirection:"column",borderLeft:"1px solid #D6E0ED",background:"#fff",overflow:"hidden" }}>
          <div style={{ padding:"12px 14px",borderBottom:"1px solid #D6E0ED",background:"#F8FAFC",flexShrink:0 }}>
            ...
            <div style={{ fontSize:10,color:"#6381A7",fontFamily:"var(--mono)",marginTop:1 }}>{fmtDate(trip?.dateStart)} {fmtTime(trip?.dateStart)} → {fmtDate(trip?.dateEnd)} {fmtTime(trip?.dateEnd)}</div>
          </div>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:5,padding:"9px 12px",borderBottom:"1px solid #EEF2F7",flexShrink:0 }}>
            {[{v:`${trip?.durationHs?.toFixed(0)}h`,l:"Duración"},{v:trip?.nServices||0,l:"Servicios",c:"#1E7A4A"},{v:`${trip?.distNm}nm`,l:"Distancia"}].map(k=>(
