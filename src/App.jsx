...
.sb-item-dot{font-size:10px;width:16px;text-align:center;flex-shrink:0;color:rgba(255,255,255,.2)}
.sb-item.active .sb-item-dot{color:rgba(255,255,255,.55)}
...
.go-btn{margin-top:16px;padding:9px 20px;border-radius:8px;background:var(--blue);color:#fff;border:none;font-family:var(--sans);font-size:12px;font-weight:600;cursor:pointer;transition:background .15s}
...
export default function App() {
  ...
  const [aisData,  setAisData]  = useState(null);   // { filename, points, trips, loadedAt }
  const [trips,    setTrips]    = useState([]);
  const [viewing,  setViewing]  = useState(null);   // trip index or null
      ...
      <div className="shell">
        ...
        <main className="main">
          ...
          <div className="page-body">
            {/* UPLOAD */}
            {tab === "Upload" && <Upload onLoad={handleLoad} />}
            ...
            {tab === "Viajes" && viewing === null && (
              aisData ? (
                <div style={{ padding:"28px 32px",maxWidth:1000 }}>
                  ...
                  <p style={{ fontSize:12,color:"var(--muted)",lineHeight:1.7,marginBottom:20,maxWidth:560 }}>
                      ...
                      <div key={t.id}
                        ...
                        <span style={{ display:"flex",gap:3,flexWrap:"wrap" }}>
                          {t.zones.slice(0,3).map(z => (
                            <span key={z} style={{ fontSize:8,padding:"1px 5px",borderRadius:3,background:"#EEF2F7",color:"#6381A7",fontFamily:"var(--mono)" }}>
                              {z.replace(/_/g," ")}
                            </span>
                          ))}
                        </span>
